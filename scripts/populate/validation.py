...
class DataValidator:
    ...
    def validate_game_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'min' in df.columns:
            ...
            if not unrealistic_min.empty:
                warnings.append(f"Found {len(unrealistic_min)} records with >60 minutes")
    ...
    def validate_boxscore_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if all(col in df.columns for col in ['field_goals_percentage', 'field_goals_attempted']):
            ...
            if not invalid_fg_pct.empty:
                errors.append(f"Found {len(invalid_fg_pct)} invalid field goal percentages")
        
        # Check three point percentages
        if all(col in df.columns for col in ['three_pointers_percentage', 'three_pointers_attempted']):
        ...
        if all(col in df.columns for col in ['free_throws_percentage', 'free_throws_attempted']):
            ...
            if not invalid_ft_pct.empty:
                errors.append(f"Found {len(invalid_ft_pct)} invalid free throw percentages")
    ...
    def validate_season_data(self, season: str, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if 'game_date' in df.columns:
            ...
            unexpected_years = [year for year in game_years if year not in expected_years]
            if unexpected_years:
        ...
        if 'season_id' in df.columns:
            ...
            unexpected_seasons = [sid for sid in season_ids if not str(sid).startswith(expected_season_id)]
    ...
    def validate_data_completeness(self, df: pd.DataFrame, expected_fields: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_completeness": 1 - (null_counts.sum() / (len(df) * len(critical_fields)))
        }
    ...
    def validate_statistical_consistency(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if all(col in df.columns for col in ['oreb', 'dreb', 'reb']):
            ...
            if not inconsistent_reb.empty:
                warnings.append(f"Found {len(inconsistent_reb)} records with inconsistent rebound totals")
        
        # Check for zero-minute players with stats
        if 'min' in df.columns:
            zero_min_with_stats = df[
                (df['min'] == 0) & 
                ((df.get('pts', 0) > 0) | (df.get('reb', 0) > 0) | (df.get('ast', 0) > 0))
            ]
            if not zero_min_with_stats.empty:
                warnings.append(f"Found {len(zero_min_with_stats)} records with 0 minutes but positive stats")
    ...
    def generate_validation_report(self, data_type: str, df: pd.DataFrame, **kwargs) -> Dict[str, Any]:
        ...
        if "expected_fields" in kwargs:
            completeness_result = self.validate_data_completeness(df, kwargs["expected_fields"])
        ...
        if "completeness" in report:
            all_validations.append(report["completeness"].get("valid", True))
        ...
        if report["overall_valid"]:
            ...
            logger.error(f"Validation failed for {data_type}: {len(validation_result.get('errors', []))} errors")
...
def validate_player_stats_consistency(player_games: pd.DataFrame) -> Dict[str, Any]:
    ...
    player_stats = player_games.groupby('player_id').agg({
        'pts': ['mean', 'std', 'count'],
        'reb': ['mean', 'std'],
        'ast': ['mean', 'std'],
        'min': ['mean', 'std']
    ...
    if not high_variation_players.empty:
        warnings.append(f"Found {len(high_variation_players)} players with high statistical variation")
    
    # Check for players with very few games but high stats (potential data errors)
    low_game_high_stats = player_stats[
        (player_stats['pts_count'] < 5) & 
        (player_stats['pts_mean'] > 25)
    ]
    
    if not low_game_high_stats.empty:
        warnings.append(f"Found {len(low_game_high_stats)} players with few games but high scoring averages")
