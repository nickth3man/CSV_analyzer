...
class PopulationManager:
    ...
    def __init__(self, client: NBAClient | None = None) -> None:
    ...
    def _is_data_type_completed(self, data_type: str, season: str) -> bool:
        ...
        return (
            data_type in self.progress["data_types_completed"]
            and season in self.progress["data_types_completed"][data_type]
            and self.progress["data_types_completed"][data_type][season]["status"]
            == "completed"
        )

    def populate_players(self, active_only: bool = False) -> pd.DataFrame:
    ...
    def populate_teams(self) -> pd.DataFrame:
        ...
        for season in seasons:
            for season_type in season_types:
                ...
                try:
                    ...
                    logger.exception(f"Error processing {season} {season_type}: {e}")
    ...
    def populate_boxscores(
        ...
        for i, game_id in enumerate(game_ids):
            ...
            try:
                ...
                logger.exception(f"Error processing box score {game_id}: {e}")
        ...
        for i, game_id in enumerate(game_ids):
            ...
            try:
                ...
                if pbp is not None and not pbp.empty:
                    logger.info(
                        f"Retrieved {len(pbp)} play-by-play events for {game_id}",
                    )
                    # Process and save play-by-play data

            except Exception as e:
                logger.exception(f"Error processing play-by-play {game_id}: {e}")
    ...
    def populate_shot_charts(
        ...
        for i, game_id in enumerate(game_ids):
            ...
            try:
                ...
                logger.exception(f"Error processing shot chart {game_id}: {e}")
        ...
        for season in seasons:
            try:
                ...
                if standings is not None and not standings.empty:
                    logger.info(
                        f"Retrieved standings for {season}: {len(standings)} records",
                    )
                    # Process and save standings data

            except Exception as e:
                logger.exception(f"Error processing standings for {season}: {e}")
            ...
            for season_type in season_types:
                try:
                    ...
                    if tracking is not None and not tracking.empty:
                        logger.info(
                            f"Retrieved tracking stats for {season} {season_type}: {len(tracking)} records",
                        )
                        # Process and save tracking data

                except Exception as e:
                    logger.exception(
                        f"Error processing tracking stats for {season} {season_type}: {e}",
                    )
            ...
            for season_type in season_types:
                try:
                    ...
                    if hustle is not None and not hustle.empty:
                        logger.info(
                            f"Retrieved hustle stats for {season} {season_type}: {len(hustle)} records",
                        )
                        # Process and save hustle data

                except Exception as e:
                    logger.exception(
                        f"Error processing hustle stats for {season} {season_type}: {e}",
                    )
