...
class DatabaseManager:
    ...
    def insert_data(self, table_name: str, df: pd.DataFrame, mode: str = "append") -> int:
    ...
    def upsert_data(self, table_name: str, df: pd.DataFrame, key_columns: List[str]) -> int:
    ...
    def get_missing_data(self, table_name: str, season: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"Error checking missing data for {table_name}: {e}")
            return []
...
def create_database_backup(db_path: Path, backup_dir: Optional[Path] = None) -> Path:
...
def restore_database_backup(backup_path: Path, target_path: Optional[Path] = None) -> Path:
    ...
    try:
        ...
        if target_path.exists():
            current_backup = target_path.parent / f"current_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.duckdb"
